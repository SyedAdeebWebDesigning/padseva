...
const HeroSection: React.FC = () => {
	...
	const leaf1X = useTransform(scrollY, [0, 1000], [0, -400]);
	const leaf1Y = useTransform(scrollY, [0, 1000], [0, -400]);
	const leaf2X = useTransform(scrollY, [0, 1000], [0, 400]);
	const leaf2Y = useTransform(scrollY, [0, 1000], [0, -400]);
	const leaf3X = useTransform(scrollY, [0, 1000], [0, -400]);
	const leaf3Y = useTransform(scrollY, [0, 1000], [0, 400]);
	const leaf4X = useTransform(scrollY, [0, 1000], [0, 400]);
	const leaf4Y = useTransform(scrollY, [0, 1000], [0, 400]);
	...
	return (
		<div className="relative h-screen bg-[#F7E7EF] overflow-hidden flex items-center justify-center">
			...
			<div className="absolute text-center z-20">
				<h1 className="text-[#91373E] font-bold oswald-300 text-9xl">
					Padseva
				</h1>
			</div>
